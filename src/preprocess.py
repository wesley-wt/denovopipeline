...
def reformat_MGF(mgf_in):
    mgf_out = mgf_in.replace(".mgf", "_reformatted.mgf")
    new_index = 1
    new_scan = 1
    with open(mgf_in) as in_f, open(mgf_out, "w") as out_f, open(
        mgf_out.replace(".mgf", "_deepnovo.mgf"), "w"
    ) as out_f2:
        for line in in_f:
            if line == "BEGIN IONS\n":
                ms2_peak_lines = []
